...
def generate_markdown():
    ...
    def print_group_stats():
        if group_stats:
            print("| |",end='')
            for suffix in suffixes:
                print(" {} |".format(str(group_stats[suffix]).rjust(suffix_max_len)), end='')
            print()
            print()
...
def main():
    parser = argparse.ArgumentParser(description='Get statstics in txt or markdown. Make sure you are in root of cosmos repo before running')
    parser.add_argument('-f','--format',help='Ouput can be \"txt\" for text file and \"md\" for markdown or \"all\" for all formats',default='all')
