...
const countUnused = str =>
  alphabet.reduce((unused, char) => unused + !str.toLowerCase().includes(char), 0)

const checkLipogram = str => {
  let unused = countUnused(str)
  
  return unused >= 2 ? 'Not a pangram but might a lipogram' : unused == 1 ? 'Pangrammatic lipogram' : 'Pangram'
}
