           ...
           #Get and assign image's RGB values
            ...
            if (i >= 2 & i < (width - 2) & j >= 2 & j < (height - 2)):
                for m in range(3):
                    ...
                    for k in range(5):
                        for l in range(5):
                            neighbour_matrice[k][l] = matris[k + i - 2][l + j - 2][m]#Create neighbor matrice's indexs(exclude a frame size=2)
                    for k in range(5):
                        for l in range(5):
                            toplam = (toplam + (neighbour_matrice[k][l] * 0.04))#0.04 is filter value.
