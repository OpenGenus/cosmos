...
struct GreaterEqual {
    bool operator()(typename _Type::iterator a,
                    typename _Type::iterator b) const
    {
        return (*a).first < (*b).first;
    }
};

void testTrueRecursive() {
    string s = "1221";
    ...
    *(iv2+4) = 1;
    ...
    assert((isPalindromeRecursive<map<int, int>::iterator, MapValueEqual, GreaterEqual<map<int, int>>>(se.begin(), se.end())));
    se[3] = 3;
    assert((isPalindromeRecursive<map<int, int>::iterator, MapValueEqual, GreaterEqual<map<int, int>>>(se.begin(), se.end())));
}

void testFalseRecursive() {
    string s = "1231";
    assert(!isPalindromeRecursive(s.begin(), s.end()));
    string s2 = "12331";
    assert(!isPalindromeRecursive(s2.begin(), s2.end()));
...
void testTrueIterative() {
    ...
    *(iv2+4) = 1;
    ...
    map<int, int> se;
    se[1] = 1; se[2] = 2; se[4] = 2; se[5] = 1;
    assert((isPalindrome<map<int, int>::iterator, MapValueEqual, GreaterEqual<map<int, int>>>(se.begin(), se.end())));
    se[3] = 3;
    assert((isPalindrome<map<int, int>::iterator, MapValueEqual, GreaterEqual<map<int, int>>>(se.begin(), se.end())));
}

void testFalseIterative() {
    string s = "1231";
    assert(!isPalindrome(s.begin(), s.end()));
    string s2 = "12331";
    assert(!isPalindrome(s2.begin(), s2.end()));
