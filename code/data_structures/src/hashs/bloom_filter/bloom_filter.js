...
(function (bfilter) {
  ...
  bfilter.BloomFilter = function BloomFilter(m, k) {
    if ((arguments.length < 2) || (undefined == m)
      || (undefined == k) || (undefined == k.length)
      || (k.length == 0) || (typeof k !== "object")) {
 ...
 */
  ...
  bfilter.BloomFilter.prototype.check = function (element) {
    ...
    if (prob) {
      return {
        "probability": calcProb(this.bitVector.length, this.hashFunctions.length, this.nbrElementsInserted), "isMember": prob
      };
    } else {
      return {
        "probability": 1,
...
function test(m, k) {
  ...
  console.log(`hello in bloom filter = ${JSON.stringify(bloomFilter.check("hello"))}`);
  console.log(`helloWorld in bloom filter = ${JSON.stringify(bloomFilter.check("helloWorld"))}`);
  console.log(`helloDear in bloom filter = ${JSON.stringify(bloomFilter.check("helloDear"))}`);
  console.log(`world in bloom filter = ${JSON.stringify(bloomFilter.check("world"))}`);
  console.log(`123 in bloom filter = ${JSON.stringify(bloomFilter.check("123"))}`);
  console.log(`Number(3456) in bloom filter = ${JSON.stringify(bloomFilter.check(3456))}`);
  console.log(`Number(3) in bloom filter = ${JSON.stringify(bloomFilter.check(3))}`);
  console.log(`Number(4) in bloom filter = ${JSON.stringify(bloomFilter.check(4))}`);
  console.log(`Number(5) in bloom filter = ${JSON.stringify(bloomFilter.check(5))}`);
  console.log(`Number(6) in bloom filter = ${JSON.stringify(bloomFilter.check(6))}`);
}
