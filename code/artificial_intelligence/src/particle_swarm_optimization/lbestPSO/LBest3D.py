...
for k in range(num_iters):
    for p in range(particles):
        ...
        distances = list(zip([distance(positions[p], other) for other in positions if (other != positions[p]).any()], range(particles)))
        distances.sort()
        ...
        for d in range(2):
            ...
            velocities[p][d] = w*velocities[p][d] + c1*r1*(best_positions[p][d] - positions[p][d]) + c1*r2*(neighbours_best[d] - positions[p][d])
