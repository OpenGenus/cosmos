...
for k in range(num_iters):
    for i in range(particles):
        ...
        distances = list(zip([distance(positions[i], other) for other in positions if (other != positions[i]).any()], range(particles)))
        distances.sort()
        ...
        velocities[i] = w*velocities[i] + c1*r1*(best_locals[i] - positions[i]) + c2*r2*(neighbours_best - positions[i])
