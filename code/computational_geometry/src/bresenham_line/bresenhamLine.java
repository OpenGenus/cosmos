...
public class bresenhamLine extends java.applet.Applet implements MouseListener, MouseMotionListener {
    ...
    */  
    public void bresenham_line(int xa, int ya, int xb, int yb) 
    {
        ...
        int sum = xb - xa;
        int Dx = 2 * (xb - xa);
        int Dy = Math.abs(2 * (yb - ya));
        int Deviation_y = ((yb - ya) > 0) ? 1 : -1;
        // draw line
    ...
    */  
    public void bresteepLine(int xc, int yc, int xd, int yd) {   
        // if point xc, yc is on the right side of point xd, yd, change them
        if ((xc - xd) > 0) {
        ...
        int sum = xd - xc;
        int Dx = 2 * (xd - xc);
        int Dy = Math.abs(2 * (yd - yc));
        int Deviation_y = ((yd - yc) > 0) ? 1 : -1;
