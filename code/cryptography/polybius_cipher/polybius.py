...
class PolybiusSquare(Cipher):
    ...
    def __init__(self, key='phqgiumeaylnofdxkrcvstzwb', size=5, chars=None):
        ...
        self.size = size
        assert len(self.key) == size * \
            size, 'invalid key in init: must have length size*size, has length ' + \
            str(len(key))
        assert len(
            self.chars) == size, 'invalid chars in init: must have length=size, has length ' + str(len(chars))
    ...
    def decipher(self, string):
        ...
        """
        string = self.remove_punctuation(
            string)  # ,filter='[^'+self.chars+']')
        ret = ''
