...
def encrypt(key, text):
    ...
    for i in range(len(msg_to_be_decrypted)):
        letter_in_key = key[
            i % len(key)
        ]  # Puts the required character of the key in variable letter_in_key
        shift = (
            alfa.find(letter_in_key) + 1
        ) % 26  # Puts the amount to be shifted in variable shift
        index_of_letter = alfa.find(
            msg_to_be_decrypted[i]
        )  # Finds the character of plaintext to be encrypted
...
def decrypt(key, encrypted):
    ...
    for i in range(len(encrypted)):
        ...
        shift = (
            alfa.find(encrypted[i]) - alfa.find(letter_in_key) - 1
        ) % 26  # Puts the amount to be shifted in variable shift
