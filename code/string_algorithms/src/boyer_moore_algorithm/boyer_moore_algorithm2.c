...
static void InitSkip(char const * const pattern, int skip[], size_t alphabetLen, int patternLen)
{
...
static const char * NextSearchBegin(const char * const curSearch, int skip)
{
...
const char * BoyerMooreSearch(char const * const txt, char const * const pattern)
{
	if (!pattern || !txt)
		return 0;
