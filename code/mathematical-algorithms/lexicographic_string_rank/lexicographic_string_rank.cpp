...
void initialize_smaller_char_cnt(int smaller_char_count[], int char_count[], string s)
{
...
ll lexicographic_string_rank(string s)
{
    ...
    initialize_smaller_char_cnt(smaller_char_count, char_count, s);      //////////// Initialize smaller_char_count array
    for(int i=0;i<256;i++){
        rep_count *= fact[char_count[i]];    //////////// Repetitive characters will cause duplicate permutations hence rank is divided by rep_count!
    }
    for(int i=0;i<len;i++){
        lex_string_rank += (fact[len-i-1] * (smaller_char_count[s[i]]) / rep_count);
        rep_count = rep_count / char_count[s[i]];
