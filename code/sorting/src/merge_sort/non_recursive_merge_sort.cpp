...
template <class T>
int Stack<T>::isFull() {
    // maximum inex we can have in array is maxsize-1 and we return 1 for isFull
    // when top is the maximum index and we can't insert any further
    if (top == maxsize - 1) {
        return 1;
    } else {
        return 0;
...
int Stack<T>::Push(T x) {
    ...
    if (isFull() == 1) {
        ...
        // increment top to next available index and set A[top] to the element
        // to be inserted
        top++;
        A[top] = x;
        return 0;
...
T Stack<T>::Pop() {
    // check for underflow
    if (isEmpty() == 1) {
        return {-1};  // TODO(unknown): better exception handling according to
                      // specifications
    } else {
        // decrement top to previous index, thus removing access to the last
        // element to enter
        T temp = A[top];
        top--;
        return temp;
...
void merge(int A[], int B[], int start, int middle, int end) {
    // initialise our iterators, i for the first half, j for the second half, k
    // for the merged aux array B
    long i = start, j = middle + 1, k = start;
    // iterating till we have used all the elements for filling aux array B
    while (k <= end) {
        // this block executed when we have unseen elements in left and right
        // half and the iterator pointing to left half is smaller
        ...
        // this block executed when we have unseen elements in left and right
        // half and the iterator pointing to right half is smaller
        if ((i <= middle && j <= end && A[i] > A[j])) {
            B[k] = A[j];
            k++;  // increment k since we filled in another element
            j++;  // increment j since we used up another element from right
                  // half
            continue;
        }
        if (i <= middle && j > end) {
...
class pairLR {
 public:
    ...
    int isNull() {
        // this function tells us whether our range is valid or not, consider it
        // to be telling us whether size of our array half is more than 1 or not
        if (start < end) {
            return 0;
        } else {
            return 1;
...
void mergeSort(int A[], int start, int end) {
    ...
    // curr is the node we are on right now, (obviously!), we initialise it with
    // entire array
    ...
    while (!S.isEmpty() || !curr.isNull()) {
        // this block lets us push to smaller left halves, something similar to
        ...
        // our curr is not valid anymore
            ...
            peek = S.Top();
            // check if the righchild (right sub array) is valid and has not
            // been visited in the previous iteration since we move towards
            // right children only in this block, safe to just check with last
            if (!rightChild(peek).isNull() &&
                !equal_To(rightChild(peek), last)) {
                curr = rightChild(peek);  // rightchild(peek) is valid and not
                                          // visited, therefore we can push it
            } else {
                // in case the right child is not valid or has been visited
                // alread (read: merged), we can take it to involve 2 sorted
...
*/
int main() {
    int t = 0, i = 0;  // t is the number of test cases, i our loop iterator
    std::cin >> t;
    while (t--) {
        int N = 0;  // N is the number of elements in array A
        std::cin >> N;
        int A[N];
